    ...
    chrome.commands.onCommand.addListener((command) => {
        switch (command) {
            ...
            case 'delete-highlight':
                trackEvent('delete-highlight', 'keyboard-shortcut');
                // This will be handled by content script
                chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
                    chrome.tabs.sendMessage(tab.id, { action: 'deleteSelectedHighlight' });
                });
                break;
            case 'open-sidepanel':
                trackEvent('open-sidepanel', 'keyboard-shortcut');
                chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
    ...
    chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
        ...
        switch (request.action) {
            ...
            case 'get-color-options':
                ...
                return true; // return asynchronously
            case 'open-sidepanel-edit':
                // Open sidepanel and pass highlightId for editing
                chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
                    chrome.sidePanel.open({ windowId: tab.windowId });
                    // Store the highlightId to be picked up by sidepanel
                    chrome.storage.local.set({ editHighlightId: request.highlightId });
                });
                return;
        }
    });
