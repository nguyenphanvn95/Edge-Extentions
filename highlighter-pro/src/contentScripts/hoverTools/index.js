...
function initializeHoverTools() {
    $.get(chrome.runtime.getURL('src/contentScripts/hoverTools/index.html'), (data) => {
        ...
        noteBtnEl = hoverToolEl.find('.highlighter--icon-note')[0];
        ...
        noteBtnEl.addEventListener('click', onNoteBtnClicked);
...
function moveToolbarToHighlight(highlightEl, cursorX) { // cursorX is optional, in which case no change is made to the x position of the hover toolbar
    ...
    const toolWidth = 144; // When changing this, also update the width in css #highlighter--hover-tools--container
...
function onNoteBtnClicked() {
    const highlightId = currentHighlightEl.getAttribute('data-highlight-id');
    // Send message to open sidepanel and edit this highlight
    chrome.runtime.sendMessage({ 
        action: 'open-sidepanel-edit', 
        highlightId: highlightId 
    });
    chrome.runtime.sendMessage({ action: 'track-event', trackCategory: 'highlight-action', trackAction: 'add-note' });
}
